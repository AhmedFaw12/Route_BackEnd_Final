...
$gender = strip_tags(trim($_REQUEST["gender"]));
$role = strip_tags(trim($_REQUEST["role"]));
...
if(empty($errors)){
    ...
    $qry = "insert into users(email, name, mobile, password, gender, role, created_by) values('$email', '$name', '$mobile', '$password', '$gender', '$role', ".$user["id"].")";
    
    $cn  = mysqli_connect(HOST_NAME, DB_USER_NAME, DB_PW, DB_NAME, PORT);
    $rslt = mysqli_query($cn, $qry);
    if($rslt){
        $_SESSION["success"] = ["User_Create"=>"Successful Creation"];
